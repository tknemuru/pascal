...
describe('calculateSnapPosition', () => {
  describe('Basic Snap (基本的なスナップ)', () => {
    it('閾値50px以内（距離30px）の場合、ターゲットの中心にスナップする', () => {
      // ターゲット: (500, 400)
      // 現在: (530, 400) → 距離 30px
  ...
  describe('No Snap (スナップしない)', () => {
    it('閾値50pxより遠い（距離60px）場合、スナップせず元の相対座標を返す', () => {
      // ターゲット: (500, 400)
      // 現在: (560, 400) → 距離 60px
      const input: SnapCalculationInput = {
        currentX: 560,
  ...
  describe('Boundary Cases (境界値)', () => {
    it('閾値ぴったり50pxの場合、スナップする（閾値を含む）', () => {
      // ターゲット: (500, 400)
      // 現在: (530, 440) → 距離 √(30² + 40²) = 50px
      const input: SnapCalculationInput = {
        currentX: 530,
        currentY: 440,
        targetX: 500,
    ...
    it('閾値をわずかに超える場合（50.1px）、スナップしない', () => {
      // ターゲット: (500, 400)
      // 現在: (530, 440.1) → 距離 ≈ 50.1px
      const input: SnapCalculationInput = {
        currentX: 530,
        currentY: 440.1,
        targetX: 500,
  ...
  describe('Realistic Scenarios (実用的なシナリオ)', () => {
    ...
    it('画面の端に投げた場合（距離200px）はスナップしない', () => {
      // 画面中央: (512, 384)
      // ドロップ位置: (700, 384)
      // 距離: 188px
      const input: SnapCalculationInput = {
