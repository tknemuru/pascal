...
import { SnapEffect } from '../effects/SnapEffect'
...
export function DraggableShape({
  ...
  const [snapEffectPosition, setSnapEffectPosition] = useState({ x: 0, y: 0 })
  ...
  const handleDragEnd = useCallback(
    (_event: MouseEvent | TouchEvent | PointerEvent, info: PanInfo) => {
      ...
      // 各ターゲットとの距離をチェック（calculateSnapPositionを使用）
      for (const target of targets) {
        if (target.placedShapeId) continue // 既に配置済みのターゲットはスキップ

        // 統一されたスナップ判定ロジックを使用
        // 型の一致、距離、回転角度をすべてチェック
        const snapResult = calculateSnapPosition({
          ...
          currentRotation,
          requiredRotation: target.requiredRotation,
        })
        ...
        if (snapResult.shouldSnap) {
          ...
          // スナップエフェクトを表示
          setSnapEffectPosition({ x: target.x, y: target.y })
          setSnapEffectTrigger(true)
  ...
  // スナップエフェクト完了時のコールバック
  const handleSnapEffectComplete = useCallback(() => {
    setSnapEffectTrigger(false)
  }, [])
  ...
  if (isPlaced) {
    ...
    return (
      <>
        ...
        <SnapEffect
          x={snapEffectPosition.x}
          y={snapEffectPosition.y}
          trigger={snapEffectTrigger}
          onComplete={handleSnapEffectComplete}
        />
      </>
    )
  }
